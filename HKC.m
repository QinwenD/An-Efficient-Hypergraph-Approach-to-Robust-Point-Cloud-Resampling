function location_resampled = HKC(location_original,resampling_ratio)
...
kernel_RMat = kernel_Location_normalized*kernel_Location_normalized.'; % Rs matrix of kernel
[V,lambda_kernel] = eig(kernel_RMat); % hypergraph spectrum basis V and its corresponding eigenvalues
...
for i1 = 1:N
 ...
 pointi_position(2)-3*d(2)/2, pointi_position(2)+3*d(2)/2, pointi_position(3)-3*d(3)/2, pointi_position(3)+3*d(3)/2]; % Region of interest of point i1
    ...
    pointi_signal_high = V*diag(1-lambda_kernel)*V'*pointi_signal; % High frequency components
    pointi_signal_low = pointi_signal-pointi_signal_high; % Low frequency components
    indicators(i1) = norm(pointi_signal_high)/norm(pointi_signal_low);
end
